                                        ...
                                        _In_ PDETOUR_CREATE_PROCESS_ROUTINEW pfCreateProcessW)
{
    ...
    PROCESS_INFORMATION pi;
    STARTUPINFOW si;
    WCHAR szExe[MAX_PATH+1] = {0};
	WCHAR szCommand[14 + MAX_PATH + 4] = {0};
    PDETOUR_EXE_HELPER helper = NULL;
    HRESULT hr;
    ...
    DWORD nLen = GetWindowsDirectoryW(szExe, MAX_PATH);
    if (nLen == 0 || nLen >= ARRAYSIZE(szExe)) {
        goto Cleanup;
    }
	...
	hr = StringCchCopyW(szCommand, ARRAYSIZE(szCommand),L"rundll32.exe \"");
	if (!SUCCEEDED(hr)) {
		goto Cleanup;
	}
	int wChs = MultiByteToWideChar(CP_ACP,0,&helper->rDlls[0],-1,&szCommand[14],MAX_PATH);
	if (0 == wChs){
		goto Cleanup;
	}

    hr = StringCchCatW(szCommand, ARRAYSIZE(szCommand),L"\",#1");
	if (!SUCCEEDED(hr)) {
		goto Cleanup;
	}
